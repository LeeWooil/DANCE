...
import torch
import os
import CBM_training.utils.cbm_utils as cbm_utils
import argparse
from torch.utils.data import DataLoader
from video_dataloader import video_utils
from CBM_training.model.learning_concept_layer import train_aggregated_classification_layer,train_classification_layer, train_cocept_layer, train_pose_cocept_layer
from CBM_training.Feature_extraction.save_features import get_multi_modal_encoder, frozen_all_parameters, get_video_encoder
import time
...
parser.add_argument('--data_set', default='Kinetics-400', choices=['kth','kth-5','kth-2','penn-action','penn-action_adding_class','haa100','kinetics100','kinetics400','kinetics400_scratch', 'mini-SSV2','SSV2', 'UCF101','UCF100', 'HMDB51','image_folder','mimetics'],
...
parser.add_argument('--new_anno_path', type=str, default=None, help='(Optional)' )
...
def train_cbm_and_save(args):
    ...
    if args.new_anno_path and args.backbone.startswith("vmae_"):
        ...
        new_train_feats_list = []
        with torch.no_grad():
            for videos, _ in loader_train:
                feats = video_encoder.forward_features(videos.to(device))
    ...
    if args.no_cbm:
        ...
        from CBM_training.model.learning_dump_concept_layer import train_non_cbm
    ...
    if args.new_anno_path and args.dual_encoder.startswith("internvid"):
        ...
        with torch.no_grad():
            for images, _ in loader_train:
                feats = dual_encoder.encode_vision(images.to(device))  # (B, D)
                new_vlm_train.append(feats.cpu())
        new_vlm_train = torch.cat(new_vlm_train, dim=0)
        ...
        with torch.no_grad():
            for images, _ in loader_val:
                feats = dual_encoder.encode_vision(images.to(device))
        ...
        # 5) concat
        vlm_features     = torch.cat([vlm_features,     new_vlm_train], dim=0)
        val_vlm_features = torch.cat([val_vlm_features, new_vlm_val],   dim=0)
        print(f"[NEWCLASS] merged vlm_features: {vlm_features.shape}")
# ────────────────────────────────────────────────────────────────────────────
