...
class UCF101_SCUBA(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        import pandas as pd
        if 'SCUBA' in self.args.data_set :
        ...
        elif self.args.data_set in ['UCF101-BG'] :
            cleaned = pd.read_csv(self.anno_path, header=None, delimiter=' ')
            # root_dir -- inpaint/ -- inpainted frames
            #          -- seg/ -- human mask frames
        ...
        if (mode == 'train'):
            ...
            self.test_video_len_array = []
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    for idx in range(len(self.label_array)):
                        sample_label = self.label_array[idx]
                        if self.video_len_array is not None :
                            sample_video_len = self.video_len_array[idx]
                            self.test_video_len_array.append(sample_video_len)
    ...
    def loadvideo_frame(self, sample, video_len) :
        ...
        buffer = []
        for frame_idx in all_index :
            frame_name = f"{str(frame_idx).zfill(6)}.jpg"
            if os.path.isfile(os.path.join(sample, frame_name)) :
                img = Image.open(os.path.join(sample, frame_name))
            else :
                frame_name = f"frame{str(frame_idx).zfill(6)}.jpg"
                img = Image.open(os.path.join(sample, frame_name))
            buffer.append(np.array(img))
            
        return np.array(buffer)
        
    def loadvideo_bg_frame(self, sample, video_len) :
        ...
        buffer = []
        for frame_idx in all_index :
            if self.args.data_set == 'UCF101-BG' :
                frame_name = f"img_{str(frame_idx).zfill(5)}.jpg"
            elif self.args.data_set == 'Kinetics-BG' :
