...
class UCFVideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        if (mode == 'train'):
            ...
            print("[DEBUG] no_aug:", self.no_aug)
            print("[DEBUG] visualize:", self.visualize)
            if self.no_aug:
                self.data_transform = video_transforms.Compose([
                    video_transforms.Resize(self.short_side_size, interpolation='bilinear'),
                    video_transforms.CenterCrop(size=(self.crop_size, self.crop_size)),
                    ClipToGrayscale(num_output_channels=1),
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            # sample = os.path.join(self.data_path,self.dataset_samples[index]+".avi")
            if self.dataset_samples[index].startswith('/') and self.dataset_samples[index].split('/')[2] == "UCF101_place365":
                video_len = len(os.listdir(self.dataset_samples[index]))
...
class ClipToGrayscale:
    def __init__(self, num_output_channels=1):
        self.num_output_channels = num_output_channels

    def __call__(self, clip):
        return [F.rgb_to_grayscale(Image.fromarray(frame), num_output_channels=self.num_output_channels) for frame in clip]