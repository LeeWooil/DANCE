...
import Concept_exraction.Motion_discovery.visualize_pose as visualzie_pose
...
parser.add_argument('--subsampling_mode', type=str, default="ver1", choices=["ver1","ver2","ver3","ver4","ver5","sim+conf","wo_cos_sim","sliding_window","non-overlapping"])
...
def concept_decovery(args):
    ...
    util.save_cluster_info(data,result_gt,num_concept,output_path, save_path)
    closest_sample_indices = labeling.labeling(args,data,result_gt,save_path,output_path,num_concept)
    visualize_pose.concept_visualize_video(args,data,result_gt,closest_sample_indices,save_path,output_path)
